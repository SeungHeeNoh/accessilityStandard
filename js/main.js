...
window.onload = function(){
	...
	var banner = document.getElementById("bannerInner");
	var bannerDt = banner.getElementsByTagName("dt");
	var bannerDd = banner.getElementsByTagName("dd");
	...
	for(var k=0; k<bannerDt.length;  k++){
		bannerDt[k].children[0].onclick = function(e){
			e.preventDefault();
			for(var h=0; h<bannerDt.length; h++){
				var dtImg = bannerDt[h].children[0].children[0];
				dtImg.src = dtImg.src.replace("_over.gif", ".gif");
				bannerDd[h].style.display = "none";
			}
			this.children[0].src = this.children[0].src.replace(".gif", "_over.gif");
			var nextNode = this.parentNode.nextSibling;
			if(nextNode.nodeType == 3){
				nextNode = nextNode.nextSibling;
			}
			nextNode.style.display="block";
